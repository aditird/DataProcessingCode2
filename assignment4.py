...
def point_query(parent_partition_table_name, utc_val, save_table_name, connection):
    ...
    query = "CREATE TABLE {} AS SELECT * FROM {} WHERE created_UTC ={};".format(save_table_name, parent_partition_table_name, utc_val)
...
def range_query(parent_partition_table_name, utc_min_val, utc_max_val, save_table_name, connection):
    ...
    """
    cursor = connection.cursor()
    query = "CREATE TABLE {} AS SELECT * FROM {} WHERE created_UTC >{} AND created_UTC <= {} ORDER BY created_UTC asc;".format(save_table_name, parent_partition_table_name, utc_min_val, utc_max_val)
    cursor.execute(query)
    cursor.close()
    connection.commit()
    # raise Exception("Function yet to be implemented!")
